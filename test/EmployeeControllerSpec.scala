...
class EmployeeControllerSpec extends Specification with Mockito {
  ...
  "Add the Particular Employee" in new WithApplication {
    val search = route(FakeRequest(POST, "/addemployee").
      withFormUrlEncodedBody("id" -> "3","name" -> "Someone","address" -> "Noidas",
  ...
  "Add the Wrong Employee" in new WithApplication {
    val search = route(FakeRequest(POST, "/addemployee").
      withFormUrlEncodedBody("id" -> "3","name" -> "","address" -> "Noidas",
        "dateOfBirth" ->"12/12/1990","dateOfJoining" -> "12/12/2015","designation" -> "UP"))
    status(search.get) must equalTo(BAD_REQUEST)
  }
  ...
  "Add the Employee with  Wrong Date" in new WithApplication {
    val search = route(FakeRequest(POST, "/addemployee").
      withFormUrlEncodedBody("id" -> "3","name" -> "akash","address" -> "Noidas",
        "dateOfBirth" ->"12/123/1990","dateOfJoining" -> "12/123/2015","designation" -> "UP"))
    status(search.get) must equalTo(BAD_REQUEST)
  }
  ...
  "Edit the Wrong Date with  Employee" in new WithApplication {
    val search = route(FakeRequest(POST, "/editemployee").
      withFormUrlEncodedBody("id" -> "3","name" -> "","address" -> "Noidas",
        "dateOfBirth" ->"12/124/1990","dateOfJoining" -> "12/12/2015","designation" -> "UP"))
    status(search.get) must equalTo(BAD_REQUEST)
  }
